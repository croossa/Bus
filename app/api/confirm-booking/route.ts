...
import Booking from "@/models/Booking"; // Import Model

export async function POST(req: NextRequest) {
  try {
    // 0. Connect to DB (Do this early)
    await connectDB();
    ...
    const finalPNR = ticketData.TicketNo || ticketData.Transport_PNR;

    console.log("🎉 Booking Confirmed! PNR:", finalPNR);

    // ---------------------------------------------------------
    // STEP 3: SAVE TO MONGODB (New Addition)
    // ---------------------------------------------------------
    try {
        await Booking.create({
            bookingRefNo: busOrderKey,    // Order Key
            transportPNR: finalPNR,       // PNR from Bus API
